class Cfp::PeopleController < ApplicationController
  ...
  def show
    ...
    if @conference.specify_availability? and not @conference.in_the_past and @person.events_in(@conference).size > 0 and @person.availabilities_in(@conference).count == 0
    ...
    if @person.public_name == current_user.email
      flash[:alert] = t("cfp.public_name_not_valid")
